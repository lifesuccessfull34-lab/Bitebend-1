...
export const INDIA_STATES_DISTRICTS: StateData[] = [
  ...
  {
    state: "Punjab",
    districts: ["Amritsar","Barnala","Bathinda","Faridkot","Fatehgarh Sahib","Fazilka","Ferozepur","Gurdaspur","Hoshiarpur","Jalandhar","Kapurthala","Ludhiana","Malerkotla","Mansa","Moga","Mohali","Muktsar","Pathankot","Patiala","Rupnagar","Sangrur","Shahid Bhagat Singh Nagar","Tarn Taran"],
  },
  {
    state: "Rajasthan",
...
export const STATE_NAMES = INDIA_STATES_DISTRICTS.map((s) => s.state);

export function getDistricts(stateName: string): string[] {
  return INDIA_STATES_DISTRICTS.find((s) => s.state === stateName)?.districts ?? [];
}
